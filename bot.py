...
==============================================
  ...
  + HTTP verify endpoint for shell scripts
==============================================
"""
...
# ─── CONFIG ───────────────────────────────────────────
BOT_TOKEN  = "8636526459:AAEgTZv1GD1P1IgvLFFI9ZUeK_qENSvsxW4"
CHANNEL_ID = "@newchannel900"
ADMIN_IDS  = [1339904459]
API_SECRET = "flash2005"   # any random word e.g. "mango123"
# ──────────────────────────────────────────────────────
...
key_db = {}
bot_app = None  # global reference to the app
...
def verify_key_sync(key: str, hwid: str) -> str:
    """Synchronous key verification for HTTP handler."""
    ...
    if stored_hwid in ("", "pending"):
        ...
        loop.run_until_complete(save_keys(bot_app))
        loop.close()
        return "OK"
...
class VerifyHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        key    = params.get("key",    [""])[0].upper().strip()
        ...
        result = verify_key_sync(key, hwid)
...
def start_http_server():
    server = HTTPServer(("0.0.0.0", 8080), VerifyHandler)
    print("🌐 HTTP verify server running on port 8080")
...
# ══════════════════════════════════════════════════════
#  VERIFICATION — called by shell script (legacy)
# ══════════════════════════════════════════════════════
